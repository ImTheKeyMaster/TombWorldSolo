(() => {
  ...
  function resolvePlayerAttack(event) {
    ...
    if (els.resolveAttackBtn.dataset.phase === "confirm") {
      ...
      if (npo.wounds === 0) npo.ready = false;
      addLog(`${npo.name} rolled ${pending.criticalSaves} critical and ${pending.normalSaves} normal saves, suffered ${pending.damage} damage, and fell from ${pending.beforeWounds} to ${npo.wounds} wounds.`);
      pendingEnemyAttack = null;
      els.attackDialog.close();
      saveAndRender();
      return;
    ...
    els.combatResultPreview.innerHTML = `
      ...
      <p class="form-note">Damage is only applied after you select Confirm Damage. Cancel leaves the NPO unchanged.</p>`;
